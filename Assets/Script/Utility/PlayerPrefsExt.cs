...
public class PlayerPrefsExt : PlayerPrefs
{
    ...
    enum ArrayType
    {
        Float, Int32, Bool, String, Vector2,
    ...
    public static bool[] GetBoolArray(String key)
    {
        if (HasKey(key))
        {
            ...
            if (bytes.Length < 5)
            {
                DebugX.LogError("Corrupt preference file for " + key);
                return new bool[0];
            }

            if ((ArrayType) bytes[0] != ArrayType.Bool)
            {
                DebugX.LogError(key + " is not a boolean array");
                return new bool[0];
            }
    ...
    public static bool SetStringArray(String key, String[] stringArray)
    {
        ...
        for (var i = 0; i < stringArray.Length; i++)
        {
            ...
            {
                DebugX.LogError("Can't save null entries in the string array when setting " + key);
                return false;
            }

            if (stringArray[i].Length > 255)
            {
                DebugX.LogError("Strings cannot be longer than 255 characters when setting " + key);
                return false;
            }
    ...
    public static String[] GetStringArray(String key)
    {
        if (HasKey(key))
        {
            ...
            {
                DebugX.LogError(key + " is not a string array");
                return new String[0];
            }
    ...
    private static void GetValue<T>(String key, T list, ArrayType arrayType, int vectorNumber, Action<T, byte[]> convert) where T : IList
    {
        if (HasKey(key))
        {
            ...
            {
                DebugX.LogError(key + " is not a " + arrayType + " array");
                return;
            }
    ...
    public static void ShowArrayType(String key)
    {
        ...
        if (bytes.Length > 0)
        {
            ...
            DebugX.Log(key + " is a " + arrayType + " array");
        }
    }
