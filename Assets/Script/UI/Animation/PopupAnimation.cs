...
public class PopupAnimation : MonoBehaviour
{
    ...
    [SerializeField]
    private PanelMoveType panelMoveType = PanelMoveType.None;

    // [HideIf("panelMoveType", PanelMoveType.None)]
    ...
    public void OpenAnimation()
    {
        ...
        if (panelMoveType != PanelMoveType.None)
        {
            ...
            if(isBounce)
                rect.DOAnchorPos(_targetPos, 0.3f).SetEase(Ease.OutBack).SetUpdate(true);
            else
    ...
    public void CloseAnimation(Action action)
    {
        ...
        rect.DOKill();
        rect.DOAnchorPos(_targetPos, 0.2f).SetEase(Ease.OutCubic).SetUpdate(true).OnComplete(() =>
        {
            action?.Invoke();
        }).SetUpdate(true);
    }
